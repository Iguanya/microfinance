<?PHP
	...
	if (isset($_GET['inc_id'])){
		...
		$query_income = db_query($db_link, $sql_income);
		...
		if($income[4] != NULL){
			...
			$query_delltrans = db_query($db_link, $sql_delltrans);
		...
		if ($income[1] == 5){
			...
			$query_revert_fined = db_query($db_link, $sql_revert_fined);
